...
const ContactForm = () => {
    ...
    return (
        <div className="max-w-sm w-[80%] mx-auto p-2 bg-transparent border-t-2 border-x-2 border-x-white/20 border-t-white/50 border-b-4 border-b-gray-900 shadow-2xl shadow-violet-700/90 rounded-lg -translate-y-6 z-50 ">
            <h2 className="text-2xl font-bold mb-4 lg:opacity-0 text-gray-500"><p className='text-white inline-block uppercase pt-4 font-extrabold gradient-text'> Contact</p> me</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 z-50">
                <div>
                    ...
                    <input {...register('name')} className="w-full border-2 p-1 bg-white/90 rounded-xl" />
                ...
                <div>
                    ...
                    <input {...register('email')} placeholder="Your Email" className="w-full border-2 p-1 bg-white/90   rounded-xl" />
                ...
                <div>
                    ...
                    <textarea {...register('message')} placeholder='....' className="w-full h-[12vh] border-2 p-1 bg-white/90  rounded-xl"></textarea>
                    {errors.message && <p className="text-red-900/80 text-xs">{errors.message.message}</p>}
                </div>
                <button disabled={isSubmitting} className="border-2 w-full p-2 text-white rounded-lg bg-gradient-to-r from-[#ff8c00] to-[#8a2be2] opacity-80 uppercase font-extrabold hover:opacity-100">
